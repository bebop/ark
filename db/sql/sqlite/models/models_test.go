...
// 	rheaPath := filepath.Join(datapath, "rhea/rhea_mini.rdf.gz")
// 	// First, test Rhea insert. We need both to test uniprot2rhea
// 	rhea, err := rhea.Read(rheaPath)
// 	if err != nil {
...
// 	uniprotSprot, errors, err := uniprot.Read(filepath.Join(datapath, "uniprot_sprot_mini.xml.gz"))
// 	if err != nil {
...
// 	err = RheaTsvInsert(db, filepath.Join(datapath, "rhea/rhea2uniprot_test.tsv.gz"), true)
// 	if err != nil {
...
// 	sequences := genbank.ReadFlatGz(filepath.Join(datapath, "flatGbk_test.seq.gz"))
...
// 	// Read Chembl schema
// 	schemaStringBytes, err := ioutil.ReadFile(filepath.Join(datapath, "chembl_schema.sql"))
// 	if err != nil {
